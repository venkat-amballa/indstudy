...
train_dataset = TripletHAM10000Dataset(csv_file=TRAIN_DIR_PTH, img_dir=IMAGES_DIR, transform=train_transforms)
val_dataset = TripletHAM10000Dataset(csv_file=VAL_DIR_PTH, img_dir=IMAGES_DIR, transform=train_transforms)
test_dataset = HAM10000Dataset(csv_file=TEST_DIR_PTH, img_dir=IMAGES_DIR, transform=test_transform)
...
# def save_results(results, test_loss, file_path):
#     results_df = pd.DataFrame(results)
#     results_df['test_loss'] = test_loss
#     results_df.to_csv(file_path, index=False)
#     print(f"Results saved to {file_path}")

# save_results(train_results, test_loss, RESULTS_PTH)
# print(f"Validation Loss after training: {test_loss:.4f}")
